...
if __name__ == '__main__':
    ...
    _sample_decoder_layer = DecoderLayer(
        d_model=_d_model,
        num_heads=_num_head,
        dff=_dff
    )

    _sample_decoder_layer_output, _, _ = _sample_decoder_layer(
        tf.random.uniform((_batch_size, _input_seq_len, _d_model)),
        enc_output=tf.random.uniform((_batch_size, _input_seq_len, _d_model)),
        training=False,
        look_ahead_mask=None,
        padding_mask=None
    )

    print(_sample_decoder_layer_output.shape)
